      ...
      <div className="relative">
        ...
        <div className="relative flex justify-between items-center">
          {phases.map((phase: Phase) => {
            ...
            return (
              <div key={phase.id} className="flex flex-col items-center flex-1 z-10">
                <div className={`w-16 h-16 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-110 border-4 border-white ${themeGradient}`}>
                  <span className="text-2xl">{iconEmoji}</span>
                </div>
                <div className="mt-4 text-center">
                  <div className="text-slate-700">{phase.name}</div>
                  <div className="text-sm text-slate-500">{phase.status}</div>
                  <div className="text-xs text-slate-400 mt-1">{phase.progress}%</div>
                </div>
              </div>
            );
          })}
