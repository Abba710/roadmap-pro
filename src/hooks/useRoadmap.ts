...
export function useRoadmap(initialPhases: Phase[]) {
   ...
   */
  const toggleMilestone = useCallback((phaseId: string, milestoneId: string): void => {
    setPhases((prevPhases: Phase[]) =>
      prevPhases.map((phase: Phase) => {
        if (phase.id === phaseId) {
          const updatedMilestones: Milestone[] = phase.milestones.map((milestone: Milestone) =>
            milestone.id === milestoneId
              ? { ...milestone, completed: !milestone.completed }
              : milestone
          );
          const completedCount: number = updatedMilestones.filter((m: Milestone) => m.completed).length;
          const progress: number = Math.round((completedCount / updatedMilestones.length) * 100);
  ...
  const getOverallProgress = useCallback((): number => {
    ...
    return Math.round(phases.reduce((acc: number, p: Phase) => acc + p.progress, 0) / phases.length);
  }, [phases]);
  ...
  const getTotalCompletedMilestones = useCallback((): number => {
    return phases.reduce(
      (acc: number, p: Phase) => acc + p.milestones.filter((m: Milestone) => m.completed).length,
      0
    );
  }, [phases]);
  ...
  const getTotalMilestones = useCallback((): number => {
    return phases.reduce((acc: number, p: Phase) => acc + p.milestones.length, 0);
  }, [phases]);
